...
if __name__ == '__main__':
    ...
    df = pd.read_csv(os.path.join(localdatapath, "asoiaf_data_cleaned.csv"), header=0, index_col = 0)
    df["names"] = df.apply(lambda r: eval(r["names"]), axis=1)

    G = character_graph(df["names"])
    print("Saving graph to file ...")
    nx.write_gpickle(G, os.path.join(datapath, "asoiaf_graph_cleaned.nxgraph"))
